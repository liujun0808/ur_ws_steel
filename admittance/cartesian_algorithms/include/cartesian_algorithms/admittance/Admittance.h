...
public:
  ...
  // 测试接口
  std::vector<double>       test_desired_pose_;
  Vector6d       arm_desired_pose_adm_;
  ros::Publisher pub_arm_cmd_stamp;
  ros::Publisher pub_ee_twist;
  ros::Publisher pub_ee_pose;
  ros::Publisher pub_arm_desired_adm_pose;
  double twist_scale_;// 速度命令发布前的 缩放系数
  geometry_msgs::TwistStamped ee_twist; // 末端速度
  geometry_msgs::PoseStamped ee_pose,pose_adm_theta;
  std::vector<double> test_K_;
  Vector6d      wrench_external_desired;
