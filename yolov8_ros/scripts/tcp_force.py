...
class UR_Robot:
    def __init__(self, tcp_host_ip="192.168.1.2", tcp_port=30003, workspace_limits=None):
        # Init varibles
    ...
    def enable_ft(self):
        ...
        tcp_command = "ft_rtde_input_enable(True)" #"movej([]),a=,v=,\n"
    ...
    def get_tcp_force(self):
        ...
        actual_tool_force = self.parse_tcp_state_data(state_data, 'TCP force')
        actual_tool_force = actual_tool_force.tolist()
        actual_tool_force_ = self.exchange(actual_tool_force)
        # print("返回的力矩:",actual_tool_force)
        return actual_tool_force_
    ...
    def parse_tcp_state_data(self, data, subpasckage):
        ...
        elif subpasckage == 'TCP force':
            tcp_force = dic["TCP force"]  # get force and torque x y z rx ry rz
            tcp_force_ = np.array(tcp_force[1])
            return tcp_force_
    ...
    def exchange(self,ft_list):
        ft_list[0] *=-1
        # ft_list[1],ft_list[2] = ft_list[2],ft_list[1]
        ft_list[1] *=-1
        # ft_list[2] *=-1

        ft_list[3] *=-1
        ft_list[4],ft_list[5] = ft_list[5],ft_list[4]
        ft_list[4] *=-1
        ft_list[5] *=-1
        return ft_list
...
if __name__ =="__main__":
    rospy.init_node("get_tcp_force")
    ur_robot = UR_Robot()
    pub = rospy.Publisher("tcp_force",WrenchStamped,queue_size=100)
    tcp_force = WrenchStamped()
    tcp_force.header.frame_id = "base_link"
    rate = rospy.Rate(500) # Hz
    while not rospy.is_shutdown():
        ft_sensor = ur_robot.get_tcp_force()
        tcp_force.header.stamp = rospy.Time.now()
        tcp_force.wrench.force.x = ft_sensor[0]/1
        tcp_force.wrench.force.y = ft_sensor[1]/(-1)
        tcp_force.wrench.force.z = ft_sensor[2]/(-1)
        tcp_force.wrench.torque.x = ft_sensor[3]/1
        tcp_force.wrench.torque.y  = ft_sensor[4]/1
        tcp_force.wrench.torque.z  = ft_sensor[5]/1
